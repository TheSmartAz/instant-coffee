...
export function useChat({
  ...
  const pendingOptionsRef = React.useRef<{ triggerInterview?: boolean } | null>(null)
  ...
  const fallbackSend = React.useCallback(
    async (content: string, options?: { triggerInterview?: boolean; generateNow?: boolean }) => {
      if (!sessionId || !content.trim()) return
      try {
        const response = (await api.chat.send(sessionId, content.trim(), {
    ...
    [finishStream, handleActionTabSwitch, onPreview, sessionId, updateMessageById]
  )

  const startStream = React.useCallback(
    (content: string, options?: { triggerInterview?: boolean; generateNow?: boolean }) => {
      if (!sessionId) return
      ...
      const eventSource = new EventSource(
        api.chat.streamUrl(sessionId, content, {
          interview: options?.triggerInterview,
          generateNow: options?.generateNow,
        })
      )
    ...
    [appendStep, fallbackSend, handleActionTabSwitch, onPreview, sessionId, stopStream, updateMessageById]
  )
  ...
  const enqueueConversation = React.useCallback(
    async (
      ...
      rawContent: string,
      options?: { triggerInterview?: boolean; generateNow?: boolean }
    ) => {
    ...
    [fallbackSend, setMessages, startStream]
  )
