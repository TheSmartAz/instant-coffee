...
from ic.tools.file import WriteFile, EditFile, _resolve, _check_sandbox
...
class DBWriteFile(WriteFile):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            if _is_product_doc(file_path):
                self._persist_product_doc(content)
    ...
    def _persist_product_doc(self, content: str) -> None:
    ...
    def _persist_html_page(self, file_path: str, content: str) -> None:
        ...
        page_svc = PageService(self._db, event_emitter=self._emitter)
        version_svc = PageVersionService(self._db, event_emitter=self._emitter)

        page = page_svc.get_by_slug(self._session_id, slug)
        if page is None:
            pages = page_svc.list_by_session(self._session_id)
            order = len(pages)
        ...
        version_svc.create_version(page.id, html=content, description="Generated by engine")
        self._db.commit()
...
class DBEditFile(EditFile):
    ...
    def _persist_html_page(self, file_path: str, content: str) -> None:
        ...
        version_svc.create_version(page.id, html=content, description="Edited by engine")
        self._db.commit()
