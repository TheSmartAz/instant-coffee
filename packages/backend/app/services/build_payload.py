"""Build payload generation — validates page schemas and provisions data models.

Extracted from the legacy graph/nodes/generate.py module. This is a
standalone utility used by the build API to prepare page schemas before
the React SSG build step.
"""
...
from .app_data_store import get_app_data_store
from .component_validator import auto_fix_unknown_components, validate_page_schema
...
async def _provision_data_model(
    ...
    try:
        ...
        logger.warning(
            "generate_build_payload failed to provision app data model",
            exc_info=True,
        )
        return None
...
async def generate_build_payload(state: Any, *, event_emitter: Any | None = None) -> dict:
    """Validate page schemas, provision data models, and return updated state."""
...
generate_node = generate_build_payload
...
def _sync_pages_with_schemas(
    ...
    try:
        ...
        logger.exception("Failed to sync pages with schemas")


def get_project_state(db_session: Any, session_id: str) -> dict[str, Any]:
    """Build a project state dict from DB for context injection.

    Returns a dict with pages, product_doc summary, and version info
    suitable for injecting into the agent's context after compaction
    or on session resume.
    """
    from .product_doc import ProductDocService

    state: dict[str, Any] = {"pages": [], "product_doc": None}

    try:
        pages = PageService(db_session).list_by_session(session_id)
        for p in pages:
            state["pages"].append({
                "slug": p.slug,
                "title": p.title,
                "version": p.current_version_id,
                "description": p.description or "",
            })
    except Exception:
        logger.debug("Failed to load pages for project state")

    try:
        doc = ProductDocService(db_session).get_by_session_id(session_id)
        if doc:
            # Truncate to first 2000 chars for summary
            content = doc.content or ""
            state["product_doc"] = content[:2000] if len(content) > 2000 else content
            state["product_doc_version"] = doc.version
            state["product_doc_status"] = doc.status
    except Exception:
        logger.debug("Failed to load product doc for project state")

    return state


__all__ = ["generate_build_payload", "generate_node", "get_project_state"]
